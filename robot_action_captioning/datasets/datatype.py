...
class FrameData(BaseModel):
    ...
    robot_state: Optional[Dict[str, np.ndarray]]
...
class EnvironmentData(BaseModel):
    ...
    def _load_from_json(cls, json_str: str):
        ...
        return cls(
            ...
            camera_height=env_kwargs.get('camera_heights'),
            camera_width=env_kwargs.get('camera_widths'),
        )
...
class EpisodeData(BaseModel):
    ...
    @classmethod
    def _load_from_json(cls, json_str: str):
        data = json.loads(json_str) # data = ep_meta

        if 'object_cfgs' in data:
            objects = []
            for obj in data['object_cfgs']:
                objects.append(ObjectInfo(
                    name=obj['name'],
                    category=obj['info']['cat']
                ))
            data['objects_info'] = objects

        return cls(**data)

    @classmethod
    def load_from_hdf5(cls, hdf5_file_path: str, episode_id: str):
        ...
        """
        with h5py.File(hdf5_file_path, 'r') as f:
            if episode_id not in f['data']: # ex) episode_id = demo_1
